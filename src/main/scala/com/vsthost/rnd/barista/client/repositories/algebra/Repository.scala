package com.vsthost.rnd.barista.client.repositories.algebra

import com.vsthost.rnd.barista.client.core.{BaristaClient, Record}
...
/**
  ...
  */
abstract class Repository[F[_], ID, A <: Record[ID]](val api: BaristaClient[F], val base: String)(implicit decoder: Decoder[A]) {
  /**
    ...
    */
  def filter(filters: BaristaClient.Params): F[List[A]] = api.getRecords(base, filters)
  ...
  /**
    ...
    * @return [[List]] of all filtered records.
    */
  def filter(filters: (String, String)*): F[List[A]] = filter(Map(filters: _*))
  ...
  /**
    ...
    */
  def all: F[List[A]] = filter(Map.empty[String, String])
}
